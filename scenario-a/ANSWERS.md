...
**Server:** `REPLACE_WITH_VPS_IP` — Ubuntu REPLACE_ME (`lsb_release -ds`)
...
| --- | --- | --- | --- | --- |
| `/srv/app` | root:root | 0755 | — | a doorway; traversable, not writable |
| `/srv/app/src` | root:devs | 2770 | `g:auditor:r-x`, `u:myappuser:r-x` | devs (incl. carol) edit; dan reads; service reads |
| `/srv/app/config` | root:ops | 2770 | `g:devs:r-x`, `g:auditor:r-x`, `u:myappuser:r-x` | ops edit, devs and dan read |
| `/srv/app/secrets` | root:ops | 0750 | `g:auditor:r-x` **on the directory only** | ops read; dan lists |
| `/srv/app/secrets/db-password.txt` | root:ops | 0640 | **none** | ops read; nobody else |
| `/srv/app/logs` | myappuser:devs | 2770 | `g:auditor:r-x` | app writes, devs and dan read |
| `/srv/app/backups` | carol:ops | 3770 + `chattr +i` on files | `g:auditor:r-x` | carol owns it and still cannot delete |

`others` is `---` on every subdirectory. Nothing is world-readable; every
cross-team read is an explicit ACL entry, so `getfacl -R /srv/app` *is* the
...
its size, owner and mode. dan has `r-x` on `/srv/app/secrets` through an ACL
entry for the `auditor` group, so he can enumerate the directory and see
`db-password.txt` with all its metadata.

Reading the *contents* is a separate check against the file's own mode. That
file is `0640 root:ops` with no ACL at all — dan is not the owner, is not in
`ops`, and "other" has no bits — so the `open()` behind `cat` is refused with
...
recursive ACL would have propagated `auditor:r` onto the file and dan could
`cat` it; a default ACL would silently do the same thing to every secret added
in future.

### Task 3 — Why the sticky bit alone does not stop carol
...
deliberately makes carol the owner of `/srv/app/backups`, so the sticky bit
exempts her and `rm` succeeds.

What actually stops her is the immutable attribute on the files:

```bash
sudo chattr +i /srv/app/backups/backup1.tar
```
...
useful work: it stops other `ops` members deleting each other's files.
...
[`configs/sudoers-myapp`](configs/sudoers-myapp) grants `%devs` a `Cmnd_Alias` of
fully-spelled-out `systemctl` invocations against `myapp` only, `NOPASSWD`.
...
Proof (rows 3, 5, 12 of the table): alice restarts `myapp`; `sudo -n apt update`
as alice is refused; dan is refused entirely because `auditor` appears nowhere
in the policy. `sudo -l -U alice` prints the exact allowed list.
...
| --- | --- | --- |
| Which PID | `sudo ss -lptn 'sport = :8080'` | `<<FILL: pid>>` |
...
| Full command line | `tr '\0' ' ' < /proc/<PID>/cmdline; echo` | `python3 -m http.server 8080` |
...
column is blank for anything I do not own, and `lsof -i :8080` prints nothing
at all plus a pile of `Permission denied` warnings. With sudo the same walk
succeeds and the PID, user and command appear. The port-9090 process I started
...
grep -r 8080 /etc/cron* /var/spool/cron/crontabs/ 2>/dev/null
```
...
```bash
sudo ss -lptn 'sport = :8080'   # no output = free
```
...
For my case, `curl localhost:8080` works but `curl http://<server-ip>:8080`
...
```bash
ss -lntp | grep 8080     # is it 127.0.0.1:8080 or 0.0.0.0:8080 ?
sudo ufw status verbose
# from the laptop:
nc -vz <server-ip> 8080          # refused vs timeout, without curl in the way
```

`ss -lntp` distinguishes the two most likely causes on its own: `127.0.0.1:8080`
means the bind address is wrong (fix the app, `HOST=0.0.0.0`), `0.0.0.0:8080`
...
| coloured summary, detailed log | ANSI only when `[ -t 1 ]`; timestamped lines to `/var/log/healthcheck.log` |
...
```
[ OK ] app-1      http://127.0.0.1:3001/healthz   200 in 3ms
...
Evidence: `crontab -l` and `tail /var/log/healthcheck.log` showing at least two
...
```
● myapp.service - myapp demo service
...
- Alert on `NRestarts` — `systemctl show myapp -p NRestarts` scraped by
...
- `journalctl -u myapp -p err --since -15m | wc -l` crossing a threshold.
...
| # | Want | Command |
| --- | --- | --- |
| 1 | Last 10 minutes | `journalctl -u myapp --since "10 min ago"` |
| 2 | Errors and worse | `journalctl -u myapp -p err` (`-p err` = priority ≤ 3, so err/crit/alert/emerg) |
| 3 | This boot / previous boot | `journalctl -u myapp -b 0` / `journalctl -u myapp -b -1` |
| 4 | JSON | `journalctl -u myapp -o json-pretty -n 5` (`-o json` for one object per line, which is what you pipe to `jq`) |
| 5 | Follow live | `journalctl -u myapp -f` then `sudo systemctl restart myapp` in another terminal |
...
logs `UNHEALTHY` at T+`<<FILL>>`s, `systemd[1]: myapp.service: Scheduled
...
| Algorithm | 3001 | 3002 | What it does |
| --- | --- | --- | --- |
...
| --- | --- | --- |
| Requests failed before 3002 was ejected | `<<FILL>>` | `<<FILL>>` |
| Time from stopping 3002 to a clean stream | `<<FILL>>s` | `<<FILL>>s` |
| Time from restarting 3002 to traffic returning | `<<FILL>>s` | `<<FILL>>s` |
